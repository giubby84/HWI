from hwilib.devices.softwarelib.IpcMessage import IpcMessage, SIGN_TX, PING, AUTHORIZE_TX
...
class SoftwareClient(HardwareWalletClient):
    ...
    def sign_tx(self, tx, auth=""):
        ...
        if sock is None:
            raise DeviceConnectionError("Unable to open a tcp socket with the softwarelib device")
    ...
    def sign_message(self, message, keypath):
        ...
        if sock is None:
            raise DeviceConnectionError("Unable to open a tcp socket with the software device")
...
def enumerate(password=''):
    ...
    d_data = {}
    d_data['type'] = 'software'
    d_data['model'] = 'checksig software wallet'
